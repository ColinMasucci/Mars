...
from ..ast import ast_nodes as ast
...
def _annotate_source_range(code, source_map, start_idx, node):
    ...
    for idx in range(start_idx, len(code)):
        source_map.setdefault(idx, position)
...
def compile_program(
    ...
    for func in class_functions or []:
        compile_node(func, code, source_map=source_map)
    ...
    for stmt in node.statements:
        if isinstance(stmt, ast.FuncDecl):
            compile_node(stmt, code, source_map=source_map)
    ...
    for stmt in component_imports or []:
        compile_statement(stmt, code, source_map=source_map)

    # Emit component parameter declarations as globals
    for decl in component_params or []:
        compile_node(decl, code, source_map=source_map)
...
def compile_statement(node, code: List[Instr], source_map=None):
    ...
    compile_node(node, code, source_map=source_map)
    if isinstance(node, expr_nodes):
        code.append(("POP",))
    _annotate_source_range(code, source_map, start_idx, node)


def compile_node(node, code: List[Instr], source_map=None):
